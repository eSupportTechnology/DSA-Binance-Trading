                    ...
                    <div class="card shadow-sm border-0">
                        <div class="card-body">
                            ...
                            <form action="{{ route('course.booking.submit') }}" method="POST" enctype="multipart/form-data" id="bookingForm">
                                ...
                                {{-- Call Center Selection in Booking Form --}}
                                <div class="col-md-12 mt-2">
                                    <label>How did you hear about us? <span class="text-danger">*</span></label>
                                    <select name="call_center_id" class="form-control" required>
                                        <option value="">Select Call Center</option>
                                        @if(isset($callCenters))
                                            @foreach($callCenters as $callCenter)
                                                <option value="{{ $callCenter->id }}"
                                                    {{ (isset($selectedCallCenterId) && $selectedCallCenterId == $callCenter->id) ? 'selected' : '' }}>
                                                    {{ $callCenter->name }} - {{ $callCenter->phone_number }}
                                                </option>
                                            @endforeach
                                        @endif
                                        <option value="other" {{ (isset($selectedCallCenterId) && $selectedCallCenterId == 'other') ? 'selected' : '' }}>
                                            Other / Social Media / Website
                                        </option>
                                    </select>

                                    @if(isset($selectedCallCenter))
                                        <div class="alert alert-info mt-2">
                                            <small>
                                                <i class="icofont-check-alt"></i>
                                                You selected: <strong>{{ $selectedCallCenter->name }}</strong>
                                                ({{ $selectedCallCenter->phone_number }})
                                            </small>
                                        </div>
                                    @elseif(isset($selectedCallCenterId) && $selectedCallCenterId == 'other')
                                        <div class="alert alert-info mt-2">
                                            <small>
                                                <i class="icofont-check-alt"></i>
                                                You selected: <strong>Other / Social Media / Website</strong>
                                            </small>
                                        </div>
                                    @endif

                                    <small class="text-muted">Please select which call center you contacted or how you heard about us</small>
                                </div>
